...
if get_samsung_price_data:
    df = kiwoo_world.get_price_data(code="005930", date="20220211")
    print(df)

# 예수금을 확인하겠다.
get_deposit = False
if get_deposit:
    deposit = kiwoo_world.get_deposit()

# 오늘 주문 했던 거래들 조희하기 (다음 날 장 거래 전까지 유효)
get_order = False
if get_order:
    orders = kiwoo_world.get_order()
...
send_order = True
if send_order:
    order_result = kiwoo_world.send_order('send_buy_order', '1001', 1, '007700', 1, 35000, '00')
...
get_balance = False
if get_balance:
    position = kiwoo_world.get_balance()
...
if set_real_reg:
    ...
    kiwoo_world.set_real_reg("1000", "", get_fid("장운영구분"), "0")


    # 현재는 fid 에 채결시간만 넣어도, 모든 정보들을 다 알 수 있다.
    fids = get_fid("체결시간")
    codes = '005930;007700;000660;' # 확인할 종목 코드들
    kiwoo_world.set_real_reg("1000", codes, fids, "0")
